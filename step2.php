...
if(!empty($submitted)) { // FORM 2 SUBMITTED
    ...
    switch($submitted){
        case get_string('email', 'tool_coursearchiver'):
            $returnurl = new moodle_url('/admin/tool/coursearchiver/step3.php', array("formdata" => serialize($courses)));
            redirect($returnurl);
            break;
        case get_string('hide', 'tool_coursearchiver'):
            $returnurl = new moodle_url('/admin/tool/coursearchiver/step4.php', array("mode" => tool_coursearchiver_processor::MODE_HIDE, "formdata" => serialize($courses)));
            redirect($returnurl);
            break;
        case get_string('archive', 'tool_coursearchiver'):
            $returnurl = new moodle_url('/admin/tool/coursearchiver/step4.php', array("mode" => tool_coursearchiver_processor::MODE_ARCHIVE, "formdata" => serialize($courses)));
            redirect($returnurl);
            break;
        default:
